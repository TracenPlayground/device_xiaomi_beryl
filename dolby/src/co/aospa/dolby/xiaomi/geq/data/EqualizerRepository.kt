...
class EqualizerRepository private constructor(private val context: Context) {
    ...
    fun getPresetOrder(): List<Int> {
        val storedOrder = sharedPreferences.getString(KEY_PRESET_ORDER, null) ?: return emptyList()
        if (storedOrder.isEmpty()) return emptyList()
        return storedOrder.split(",").mapNotNull { it.trim().toIntOrNull() }
    }

    fun savePresetOrder(order: List<Int>) {
        sharedPreferences.edit().putString(KEY_PRESET_ORDER, order.joinToString(",")).apply()
    }
    ...
    fun getAllPresets(): List<Preset> {
        ...
        val order = getPresetOrder()
        if (order.isNotEmpty()) {
            val orderMap = order.mapIndexed { index, id -> id to index }.toMap()
            presets.sortWith(compareBy(
                { orderMap[it.id] ?: (Int.MAX_VALUE / 2 + it.id) }
            ))
        }
    ...
    fun addPreset(name: String): Preset {
        ...
        val currentOrder = getPresetOrder().toMutableList()
        if (currentOrder.isNotEmpty() && !currentOrder.contains(nextId)) {
            currentOrder.add(nextId)
            editor.putString(KEY_PRESET_ORDER, currentOrder.joinToString(","))
        }
    ...
    fun deletePreset(presetId: Int) {
        if (presetId >= CUSTOM_PRESET_BASE_ID) {
            ...
            val currentOrder = getPresetOrder().toMutableList()
            if (currentOrder.remove(presetId)) {
                editor.putString(KEY_PRESET_ORDER, currentOrder.joinToString(","))
            }
    ...
    companion object {
        ...
        private const val KEY_PRESET_ORDER = "dolby_preset_order"
