...
class ProfileRepository private constructor(private val context: Context) {
    ...
    private fun getProfileOrder(): List<Int> {
        val stored = sharedPreferences.getString(KEY_PROFILE_ORDER, null) ?: return emptyList()
        if (stored.isEmpty()) return emptyList()
        return stored.split(",").mapNotNull { it.trim().toIntOrNull() }
    }

    fun saveProfileOrder(order: List<Int>) {
        sharedPreferences.edit().putString(KEY_PROFILE_ORDER, order.joinToString(",")).apply()
    }
    ...
    fun getAllProfiles(): List<DolbyProfile> {
        ...
        if (order.isNotEmpty()) {
            ...
            val ordered = mutableListOf<DolbyProfile>()
            for (id in order) {
                profileMap[id]?.let { ordered.add(it) }
            }
            for (p in profiles) {
                if (!ordered.any { it.id == p.id }) {
                    ordered.add(p)
                }
            }
            return ordered
    ...
    fun createCustomProfile(name: String, baseProfileId: Int = 0): DolbyProfile {
        ...
        val order = getProfileOrder().toMutableList()
        if (order.isNotEmpty() && !order.contains(nextId)) {
            order.add(nextId)
            editor.putString(KEY_PROFILE_ORDER, order.joinToString(","))
        }
    ...
    companion object {
        ...
        private const val KEY_PROFILE_ORDER = "dolby_profile_order"
